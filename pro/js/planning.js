...
// Encaisse un RDV (reproduit _pay() de pos_screen.dart, cas simplifie 1 service).
// Cree une transaction + credite la fidelite si le client est un vrai compte Dambou +
// marque le RDV paye, exactement comme le fait la caisse mobile.
export async function checkoutBooking(booking, business, method) {
    const svc = booking.services;
    const price = svc ? (svc.price || 0) : (booking.price || 0);
    const customerId = booking.customer_id || null;
    const customerName = clientName(booking);

    const ticketNum = 'TKT-' + toDateKey(new Date()).replace(/-/g, '') + '-' + Date.now().toString().slice(-6);
    const insertData = {
        business_id: business.id,
        items: [{ name: svc ? svc.name : 'Rendez-vous', qty: 1, unit_price: price, total: price }],
        subtotal: price,
        discount: 0,
        discount_type: null,
        loyalty_discount: 0,
    ...
    const { error: txnError } = await supabase.from('transactions').insert(insertData);
    if (txnError) throw txnError;

    // Fidelite : uniquement pour les vrais comptes Dambou (comme le mobile)
    if (customerId && business.loyalty_enabled) {
        try {
            const ptsPerEuro = business.loyalty_points_per_euro || 1;
            const pointsEarned = Math.round(price * ptsPerEuro);
            const { data: existingCard } = await supabase
                .from('loyalty_cards').select('*')
                .eq('business_id', business.id).eq('customer_id', customerId).maybeSingle();

            if (!existingCard) {
                const { data: newCard } = await supabase.from('loyalty_cards').upsert({
                    business_id: business.id, customer_id: customerId,
                    points: pointsEarned, stamps: 1, total_visits: 1, total_spent: price,
                ...
                await supabase.from('loyalty_transactions').insert({
                    ...
                    type: 'earn', points: pointsEarned, stamps: 1, amount_spent: price,
                    description: 'Rendez-vous encaisse',
                });
            } else {
                await supabase.from('loyalty_cards').update({
                    points: (existingCard.points || 0) + pointsEarned,
                    stamps: (existingCard.stamps || 0) + 1,
                    total_visits: (existingCard.total_visits || 0) + 1,
                    total_spent: (existingCard.total_spent || 0) + price,
                    updated_at: new Date().toISOString(),
                }).eq('id', existingCard.id);
                await supabase.from('loyalty_transactions').insert({
                    loyalty_card_id: existingCard.id, business_id: business.id, customer_id: customerId,
                    type: 'earn', points: pointsEarned, stamps: 1, amount_spent: price,
                    description: 'Rendez-vous encaisse',
                });
            }
        } catch (e) {
            console.error('Erreur credit fidelite:', e);
        }
    }

    const { error: bkError } = await supabase.from('bookings')
        .update({ is_paid: true, payment_method: method, status: 'confirmed' })
        .eq('id', booking.id);
